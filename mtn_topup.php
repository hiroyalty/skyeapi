...
$destmsisdn = isset($_REQUEST['destmsisdn']) ? $_REQUEST['destmsisdn'] : "2347062385282";
$amount = isset($_REQUEST['amount']) ? urlencode($_REQUEST['amount']) : "100";
...
foreach ($sqvalue as $key => $value) {
    $sequenceid = (string)$value->NEXTVAL;
    #echo $sequenceid.'<br/>';
} 
#$sequenceid = "155";
#$xml = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="http://axiom.service.quickstart.samples/xsd"><soapenv:Header/><soapenv:Body><vend><origMsisdn>'.$origmsisdn.'</origMsisdn><destMsisdn>'.$destmsisdn.'</destMsisdn><amount>'.$amount.'</amount><sequence>'.$sequenceid.'</sequence><tarifftypeId>'.$tarifftypeid.'</tarifftypeId><serviceproviderId>'.$serviceproviderid.'</serviceproviderId></vend></soapenv:Body></soapenv:Envelope>';
...
$xml = '<?xml version="1.0" encoding="utf-8"?><soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">.
  ...
  <soapenv:Body>
<vend>
    <origMsisdn>' .$origmsisdn.'</origMsisdn>
    <destMsisdn>' .$destmsisdn.'</destMsisdn> 
    <amount>'.$amount.'</amount>
    <sequence>'.$sequenceid.'</sequence>
    <tarifftypeId>'.$tarifftypeid.'</tarifftypeId>
